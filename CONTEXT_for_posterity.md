...
*Section may move later in build order — new requirements are likely to emerge as Content Builder and User Profile builds proceed. Same applies to the Onboarding FAQ. Revisit timing before Stage 4 build begins.*
...
Why phase transitions notify the team: phase entries are operational handoff points. When an account enters Active Phase, the team confirms Buffer is connected, backup credentials are live, and the calendar generated correctly. Twilight entry signals the trusted contact notification flow needs to fire correctly and that TC payment pathways need to be ready. Posterity Phase entry restructures the dashboard into archive mode. Phase entries are also moments where automation could fail silently — flagging them gives the team a visible checkpoint even when no action is required.
...
- Claude.ai access to CONTEXT_for_posterity.md — investigate whether the existing MCP stack can expose the live .md to Claude.ai automatically. Currently only the rendered context PDF reaches Claude via the Project Files panel, which lags the .md. Resolve during the upcoming MCP review.
- Trust fund model brainstorm — current upfront-payment + Mercury Bank trust architecture vs. alternative year-by-year billing (charged each January 1 as plan year activates). Full ramifications, customer trust signal impact, refund obligation changes, legal posture, and marketing implications all to be settled in a dedicated session. Pencilled in before Stage 5 financial build.
- Trusted contact payment authority — TC can pay the non-recurring storage fee in two cases: (1) maintaining an account that has transferred to Posterity Phase, (2) covering a customer-skipped plan year. Storage is otherwise recurring (Horizon $9.99/year). Build the non-recurring storage option in Stripe + PayPal alongside Stage 3 pricing updates. Tighten terminology before build.
- Trusted contact plan-funding authority for unfinished accounts — a customer can pass during Horizon Phase (account exists but no plan ever funded). Trusted contact needs the ability to fund plans on the customer's behalf so the legacy can still deliver. Requires a second trigger type — current non-response trigger assumes the account has been initiated. A Horizon-phase trusted contact trigger needs to be defined: TC submits confirmation during Horizon Phase, account remains in Horizon, TC gains plan-funding authority, account proceeds through phases once plans are funded. Trigger logic, consent flow, payment authority, and phase routing all need a dedicated brainstorm.
...
Work through full context until everything matches the founder's vision.
...
## Build Roadmap (pending file recovery)
*The full build order document that lived above stage-level detail is currently missing from context. Jeremy will search his files to recover it. Until then, this slot is reserved. If the original cannot be recovered, the roadmap will be rebuilt from the older context PDF + memory.*
...
- Trust fund model brainstorm prep (Tabled & Shelved item)
...
- scripts/generate-stylized-pdf.js — styled investor PDF, manual only via npm run generate-stylized. Output: C:\Users\jerth\posterity\Project Documents\Posterity_Stylized_Context.pdf
